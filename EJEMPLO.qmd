...
Caracterizar a través de variables epidemiológicas, clínicas y del diagnóstico etiológico los casos de IRAG e IRAG extendida (IRAGe) en pacientes pediátricos del Hospital `r establecimiento` de la provincia de Mendoza, desde la SE `r semana_min_anio_minimo` de `r anio_minimo` a la SE`r semana_max_anio_minimo` de `r anio_maximo`.
...
**Figura 1.** Flujograma del circuito de datos de la UC-IRAG, Hospital `r establecimiento`.
...
Fuente: Elaboración propia, UC-IRAG Hospital `r establecimiento`.
...
Desde que se inicio la Estrategia de Unidades Centinela de Infecciones Respiratorias Agudas Graves (UC-IRAG), en el Hospital `r establecimiento` en la SE `r semana_min_anio_minimo` de `r anio_minimo` hasta la SE `r semana_max_anio_minimo` de `r anio_maximo`, se han notificado un total de 1619 casos. El siguiente gráfico muestra los casos totales de IRAG (1241) e IRAGe (378) registrados en dicho periodo. Se observa que, en ambos años, la mayor cantidad de casos se notificaron en la SE 33 (mes de agosto). Además, el año en curso presenta un menor número de casos registrados en comparación con el mismo periodo del año anterior.

**Gráfico 1.** Casos de IRAG e IRAGe por Semana Epidemiológica. Desde SE`r semana_min_anio_minimo`/`r anio_minimo` - SE`r semana_max_anio_minimo`/`r anio_maximo`. Hospital `r establecimiento`, Mendoza.
...
Fuente: datos extraídos del Sistema Nacional de Vigilancia, SNVS 2.0.
...
**Gráfico 2.** Determinaciones virales positivas por SE entre los casos de IRAG e IRAGe. Desde SE`r semana_min_anio_minimo`/`r anio_minimo` - SE`r semana_max_anio_minimo`/`r anio_maximo`. Hospital `r establecimiento`, Mendoza.
...
**Gráfico 3.** Determinaciones positivas de virus respiratorios por grupo etario en IRAG e IRAGe. Desde SE`r semana_min_anio_minimo`/`r anio_minimo` - SE`r semana_max_anio_minimo`/`r anio_maximo`. Hospital `r establecimiento`, Mendoza.
...
**Tabla 1.** Distribución de las comorbilidades en IRAG e IRAGe. Desde SE`r semana_min_anio_minimo`/`r anio_minimo` - SE`r semana_max_anio_minimo`/`r anio_maximo`. Hospital `r establecimiento`, Mendoza.
...
**Gráfico 5.** Vacunación materna VSR en menores de 6 meses notificados en la UC-IRAG (IRAG e IRAGe). Desde SE`r semana_min_anio_minimo`/`r anio_minimo` - SE`r semana_max_anio_minimo`/`r anio_maximo`. Hospital `r establecimiento`, Mendoza.
...
**Gráfico 6.** Porcentaje de IRAG e IRAGe sobre el total de internaciones hospitalarias. Desde SE`r semana_min_anio_minimo`/`r anio_minimo` - SE`r semana_max_anio_minimo`/`r anio_maximo`. Hospital `r establecimiento`, Mendoza.
...
Fuente: base de datos de la notificación agrupada numérica semanal de la UC-IRAG Hospital `r establecimiento`, Mendoza.
